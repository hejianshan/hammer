...
angular.module('stockDogApp')
  .controller('DashboardCtrl', function ($scope, WatchlistService,
    QuoteService) {
      var unregisterHandlers = [];
      $scope.watchlists = WatchlistService.query();
      $scope.ccsStyle = 'height: 300px';
      var formatters = {
        number: [
          {
            columnNum: 1,
      ...
      var updateCharts = function () {
        ...
        _.each($scope.watchlists, function (watchlist) {
          donutChart.data.push([watchlist.name,
            parseFloat(watchlist.marketValue)]);
      ...
      var reset = function () {
        QuoteService.clear();
        _.each($scope.watchlist, function (watchlist) {
      ...
      var recalculate = function () {
        ...
        _.each($scope.watchlists, function (watchlist) {
          $scope.marketValue += 
            watchlist.marketValue ? watchlist.marketValue : 0;
          $scope.dayChange += 
            watchlist.dayChange ? watchlist.dayChange : 0;
        });
        updateCharts();
      };
